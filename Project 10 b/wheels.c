...
void Wheel_Control(void) {
    ...
    LEFT_FORWARD_SPEED = LEFT_VERY_SLOW - (error*10);
    RIGHT_FORWARD_SPEED = RIGHT_VERY_SLOW + (error*100);
    LEFT_REVERSE_SPEED = WHEEL_OFF;
    RIGHT_REVERSE_SPEED = WHEEL_OFF;
    if (LEFT_VERY_SLOW - (error*10) > 50000) {
        LEFT_FORWARD_SPEED = 50000;
    }
    if (RIGHT_VERY_SLOW + (error*100) > 5000) {
