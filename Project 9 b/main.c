...
//------------------------------------------------------------------------------
  while(ALWAYS) {                       // Can the Operating system run
    ...
    switch(USB_tx_check) {
    case TRUE:
        Transmit_PC(Process_Buffer_USB);
        break;
    default: break;
    }
    /*
...
void USB_To_Buffer(void) {
    if (usb_rx_rd != usb_rx_wr) {
        ...
        Process_Buffer_USB[buffer_row_usb][direct_usb] = IOT_2_PC[iot_rx_rd++];
        if (Process_Buffer_USB[buffer_row_usb][direct_usb++] == LINE_FEED) {
            buffer_row_usb ++;
            direct_usb = BEGINNING;
            if (buffer_row_usb > 3) {
...
void Intercept_Transmit(unsigned char tx_hold[34]) {
    ...
    case TRUE:
        ...
        break;
        case FALSE:
            strcpy(tx_iot, tx_hold);
            strcpy(tx_buffer, tx_hold);
            UCA1IE |= UCTXIE;
            UCA0IE |= UCTXIE;
            break;
        default: break;
