...
void USB_To_Buffer(void) {
    while (usb_rx_rd != usb_rx_wr) { //usb is UCA1 rx
        if (direct_usb >= sizeof(Process_Buffer_USB[0])) { //if process buffer counter is past array
            ...
            if (buffer_row_usb > 3) { //if exceeds bounds
...
void Move_Process_Buffer_USB(void) {
  ...
  for (i = 0; i < 3; i++) {
    ...
    clear_row(Process_Buffer_USB, 3);
    if (buffer_row_usb != 0) {
        buffer_row_usb --;
  }
}
