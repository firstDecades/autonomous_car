...
void IOT_To_Buffer(void) {
    if (iot_rx_rd != iot_rx_wr) {
        ...
        if (direct_iot >= 34) {
            ...
            clear_row(Process_Buffer_USB, buffer_row);
        }
        Process_Buffer_USB[buffer_row_usb][direct_iot] = IOT_2_PC[iot_rx_rd++];
        if (Process_Buffer_USB[buffer_row_usb][direct_iot++] == LINE_FEED) {
            rx_check = TRUE;
            buffer_row_usb++;
            direct_iot = BEGINNING;
            if (buffer_row_usb > 3) {
...
void Transmit_IOT(unsigned char process_buf[4][34], unsigned int* buffer_row) {
    unsigned int i = 0;
    ...
    Move_Process_Buffer(process_buf, buffer_row);
    UCA0IE |= UCTXIE;
    //IOT_tx_check = FALSE;
}
void Transmit_PC(unsigned char process_buf[4][34], unsigned int* buffer_row) {
    unsigned int i = 0;
    tx_index = BEGINNING;
    tx_iot_index = BEGINNING;
    for (i = 0; i < 34; i++) {
...
void Move_Process_Buffer(unsigned char process_buf[4][34], unsigned int* buffer_row) {
    unsigned int i;
    unsigned int j;
    for (i = 0; i < 3; i++) {
        for (j = 0; j < 34; j++) {
