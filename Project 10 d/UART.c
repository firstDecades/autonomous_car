...
//global
volatile unsigned char IOT_2_PC[80] = "\0";
volatile unsigned char PC_2_IOT[80] = "\0";
...
unsigned char ip_string[21] = "\0";
unsigned int ip_index = BEGINNING;
unsigned int collectIP = FALSE;
unsigned int displayIP = FALSE;
unsigned int getIP_check = FALSE;

unsigned int ping_index = 0;
unsigned char ping[26] = "AT+PING=\"www.google.com\"\r\n";
...
__interrupt void eUSCI_A0_ISR(void) {
    ...
    case 2:
        ...
        if (collectIP == TRUE) {
            ip_string[ip_index++] = IOT_2_PC[temp];
            if (IOT_2_PC[temp] == LINE_FEED) {
                collectIP = FALSE;
                displayIP = TRUE;
    ...
    case 4:
        ...
        //break;
        if (pinger == TRUE) {
            UCA0TXBUF = ping[ping_index++];
            if (ping_index >= sizeof(ping)) {
                pinger = FALSE;
                ping_index = 0;
                UCA0IE &= ~UCTXIE;
            }
