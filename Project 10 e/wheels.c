...
/*run_forward(unsigned int period)
 * arguments: unsigned int period -> likely task_list[0].period
 * description: move forward while period is not zero
 */
void run_forward(unsigned int period) {
    if (period > 0) {
        RIGHT_FORWARD_SPEED = RIGHT_FAST;
        LEFT_FORWARD_SPEED = LEFT_VERY_SLOW+3000;
...
void run_backward(unsigned int period) {
    if (period > 0) {
        ...
        LEFT_REVERSE_SPEED = LEFT_VERY_SLOW+3000;
        LEFT_FORWARD_SPEED = WHEEL_OFF;
        RIGHT_FORWARD_SPEED = WHEEL_OFF;
        P6OUT |= GRN_LED;
        LCD_BACKLITE_DIMING = PERCENT_80;
...
void Wheels_Process(void) {
    ...
    case RUN:
        ...
        case 2:
            black_line_check();
            LEFT_FORWARD_SPEED = RIGHT_FAST;
            RIGHT_FORWARD_SPEED = LEFT_VERY_SLOW;
            LEFT_REVERSE_SPEED = WHEEL_OFF;
            RIGHT_REVERSE_SPEED = WHEEL_OFF;
...
void Wheel_Control(void) {
    ...
    LEFT_FORWARD_SPEED = LEFT_VERY_SLOW - (error*100);
    RIGHT_FORWARD_SPEED = LEFT_FAST + (error*100);
    LEFT_REVERSE_SPEED = WHEEL_OFF;
    RIGHT_REVERSE_SPEED = WHEEL_OFF;
    if (LEFT_VERY_SLOW - (error*100) > 50000) {
        LEFT_FORWARD_SPEED = 50000;
        P6OUT |= GRN_LED;
    } else {
        P6OUT &= ~GRN_LED;
    }
    if (LEFT_VERY_SLOW - (error*100) < 20000) {
        LEFT_FORWARD_SPEED = 20000;
        P6OUT |= GRN_LED;
    } else {
        P6OUT &= ~GRN_LED;
    }
    if (RIGHT_SLOW + (error*100) > 50000) {
        RIGHT_FORWARD_SPEED = 50000;
        P1OUT |= RED_LED;
    } else {
        P1OUT &= ~RED_LED;
    }
    if (RIGHT_SLOW + (error*100) < 20000) {
